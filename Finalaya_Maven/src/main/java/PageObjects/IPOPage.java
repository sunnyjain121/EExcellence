...
//    private final String csspath_IPOForthcoming_TableNoData = ".bordered>tbody>tr";
	...
	public boolean verifyForthcomingTabRecords() {
        ...
        try {
            ...
            List<WebElement> td = we_IPOForthcoming_TableNoData;
            int col_position = 0;
            for (int i = 0; i < td.size(); i++) {
            	WebElement span = td.get(i).findElement(By.tagName("span"));
				if (td.size()>0 && (!("There is no Forthcoming IPO available.").equals(span.getText()))) {
					col_position = i + 1;
					result=true;
					LoggerInstance.logger.info("Records are displaying");
					break;
				} else if (td.size()==1 && (("There is no Forthcoming IPO available.").equals(span.getText()))) {
					col_position = i + 1;
					result=true;
